...
# FIX why eps is same for everyday?
    ...
    if getattr(args, 'div', False):
        ...
        # Calculate annual yield:
                annual_yield = div_df.groupby(div_df['Date'].dt.year)['Yield (%)'].sum()

                # Identify the last dividend date in each year
                last_dates = div_df.groupby(div_df['Date'].dt.year)['Date'].transform('max')

                # Mark rows that are the last dividend entry of their year
                is_last_dividend = div_df['Date'] == last_dates

                # Map the annual yield to only the last dividend entry of each year
                div_df['Annual Yield'] = np.where(
                    is_last_dividend,
                    div_df['Date'].dt.year.map(annual_yield),
                    np.nan
                )

        # Calculate average of annual yields and present in topmost row:
        ...
        # Calculate diff from average:
                # Reuse: average of full annual yields
                avg_annual_yield = div_df['Annual Yield'].mean()

                # Prepare new column
                div_df['Projected Difference from Average'] = np.nan

                # Get year from date
                div_df['Year'] = div_df['Date'].dt.year

                # Group by year to calculate projected yield and difference
                for year, group in div_df.groupby('Year'):
                    is_last_row = group.index[-1]
                    n_divs = group['Yield (%)'].count()
                    yield_sum = group['Yield (%)'].sum()

                    # Projected yield = (YTD yield / quarters so far) * 4
                    projected_yield = (yield_sum / n_divs) * 4 if n_divs < 4 else yield_sum
                    difference = projected_yield - avg_annual_yield

                    div_df.loc[is_last_row, 'Projected Difference from Average'] = difference
